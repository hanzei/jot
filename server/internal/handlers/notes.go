...
type UpdateNoteItem struct {
	...
	Completed   bool   `json:"completed"`
	IndentLevel int    `json:"indent_level"`
	// AssignedTo is nullable; null (or omitted) clears assignment for this item.
...
func (h *NotesHandler) createTodoItems(ctx context.Context, noteID string, items []CreateNoteItem) (int, error) {
	for _, item := range items {
		...
		if _, err := h.noteStore.CreateItemWithCompleted(ctx, noteID, item.Text, item.Position, item.Completed, item.IndentLevel, nil); err != nil {
...
func normalizeAssignedTo(assignedTo *string) *string {
	if assignedTo == nil {
		return nil
	}
	if *assignedTo == "" {
...
func (h *NotesHandler) validateItemAssignments(ctx context.Context, noteID string, items []UpdateNoteItem) (int, error) {
	hasAssignment := false
	for _, item := range items {
		if normalizeAssignedTo(item.AssignedTo) != nil {
			hasAssignment = true
			break
		}
	}
	...
	for _, item := range items {
		assignedTo := normalizeAssignedTo(item.AssignedTo)
		if assignedTo == nil {
			continue
		}
		...
		if _, ok := accessSet[*assignedTo]; !ok {
...
func (h *NotesHandler) updateTodoItems(ctx context.Context, noteID string, userID string, items []UpdateNoteItem) error {
	...
	if currentNote.NoteType == models.NoteTypeTodo {
		...
		for _, item := range items {
			_, err := h.noteStore.CreateItemWithCompleted(ctx, noteID, item.Text, item.Position, item.Completed, item.IndentLevel, normalizeAssignedTo(item.AssignedTo))
			if err != nil {
				return err
			}
		}
