...
type NoteItemUpdateInput struct {
	Text        string
	Position    int
	Completed   bool
	IndentLevel int
...
func (s *NoteStore) Update(id string, userID string, title, content string, pinned, archived bool, color string, checkedItemsCollapsed bool) error {
	return s.UpdateWithItems(id, userID, title, content, pinned, archived, color, checkedItemsCollapsed, nil, false)
}

func (s *NoteStore) UpdateWithItems(
	id string,
	userID string,
	title string,
	content string,
	...
	if currentNote.Pinned != pinned {
		if pinned {
			...
			var maxPosition int
			posQuery := `SELECT COALESCE(MAX(position), -1) FROM notes
			             WHERE user_id = ? AND pinned = ? AND archived = FALSE AND deleted_at IS NULL AND id != ?`
			if err = tx.QueryRow(posQuery, userID, pinned, id).Scan(&maxPosition); err != nil {
				return fmt.Errorf("failed to get max position: %w", err)
			}
			newPosition := maxPosition + 1

			posUpdateQuery := `UPDATE notes SET position = ?, unpinned_position = ? WHERE id = ?`
			if _, err = tx.Exec(posUpdateQuery, newPosition, currentNote.Position, id); err != nil {
			...
			if _, err = tx.Exec(posUpdateQuery, targetPosition, id); err != nil {
				return fmt.Errorf("failed to update position: %w", err)
			}
		}
	}

	if replaceItems && currentNote.NoteType == NoteTypeTodo {
		...
		for _, item := range items {
			...
			if _, err = tx.Exec(
				`INSERT INTO note_items (id, note_id, text, position, completed, indent_level)
				 VALUES (?, ?, ?, ?, ?, ?)`,
