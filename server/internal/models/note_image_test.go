...
// newTestNoteImageStore opens a fresh migrated database for driver and
// returns a noteStore bound to it, along with an owning user ID and a note ID
// to attach images to.
func newTestNoteImageStore(t *testing.T, driver string) (*noteStore, string, string) {
	t.Helper()
	...
	ctx := t.Context()
	_, err := db.ExecContext(ctx, d.RewritePlaceholders(`INSERT INTO users (id, username, password_hash) VALUES (?, ?, ?)`), "user0000000000000000im", "imgowner", "x")
...
func TestNoteImageStore(t *testing.T) {
	for _, driver := range dbtest.Drivers() {
		t.Run(driver, func(t *testing.T) {
			...
			t.Run("GetNoteImageByID returns ErrNoteImageNotFound for an unknown ID", func(t *testing.T) {
				store, _, _ := newTestNoteImageStore(t, driver)
			...
			t.Run("GetNoteImagesByNoteID returns empty slice for a note with no images", func(t *testing.T) {
				store, _, noteID := newTestNoteImageStore(t, driver)
			...
			t.Run("GetNoteImageRefCount counts rows referencing a hash and drops to zero once all are deleted", func(t *testing.T) {
				...
				_, err = store.db.ExecContext(ctx, store.d.RewritePlaceholders(`INSERT INTO notes (id, user_id, note_type) VALUES ('note000000000000000im2', ?, 'text')`), userID)
				require.NoError(t, err)
	...
	for _, driver := range dbtest.Drivers() {
		t.Run(driver, func(t *testing.T) {
			...
			t.Run("EmptyTrash returns the deduped union of image hashes across all purged notes", func(t *testing.T) {
				...
				_, err := store.db.ExecContext(ctx, store.d.RewritePlaceholders(`INSERT INTO notes (id, user_id, note_type) VALUES ('note00000000000empty2', ?, 'text')`), userID)
				require.NoError(t, err)
			...
			t.Run("DeleteAllByUser returns the image hashes across all of the user's notes", func(t *testing.T) {
				...
				_, err := store.db.ExecContext(ctx, store.d.RewritePlaceholders(`INSERT INTO notes (id, user_id, note_type) VALUES ('note0000000000allbyu2', ?, 'text')`), userID)
				require.NoError(t, err)
			...
			t.Run("PurgeOldTrashedNotes only purges notes past the cutoff and returns their image hashes", func(t *testing.T) {
				...
				_, err := store.db.ExecContext(ctx, store.d.RewritePlaceholders(`INSERT INTO notes (id, user_id, note_type) VALUES ('note00000000000recent', ?, 'text')`), userID)
				require.NoError(t, err)
				...
				// Backdate only the first note's deleted_at so it alone is past the
				// cutoff. Computed in Go, like PurgeOldTrashedNotes' own cutoff, rather
				// than a dialect-specific SQL date function.
				_, err = store.db.ExecContext(ctx, store.d.RewritePlaceholders(`UPDATE notes SET deleted_at = ? WHERE id = ?`), time.Now().Add(-10*24*time.Hour), noteID)
				require.NoError(t, err)
...
func TestGetNoteImageSHA256sForUserTx(t *testing.T) {
	for _, driver := range dbtest.Drivers() {
		t.Run(driver, func(t *testing.T) {
			store, ownerID, ownedNoteID := newTestNoteImageStore(t, driver)
			ctx := t.Context()
