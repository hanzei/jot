...
// listFileNames returns the base name of every regular file under dir
// (recursively), or an empty slice if dir does not exist yet.
func (s *blobStore) listFileNames(dir string) ([]string, error) {
	var names []string
	walkErr := fs.WalkDir(s.root.FS(), dir, func(_ string, d fs.DirEntry, err error) error {
		if err != nil {
			if errors.Is(err, fs.ErrNotExist) {
				return nil // dir does not exist yet
			}
			return err
		}
		if d.IsDir() {
	...
	if walkErr != nil {
		return nil, fmt.Errorf("list files under %s: %w", dir, walkErr)
	}
	return names, nil
}
