...
// List returns the hex-encoded SHA-256 hash of every original blob currently
// stored, for use by the orphan sweep (internal/blobgc). Thumbnails are not
// listed separately — they have no independent lifecycle, since Delete
// always removes an original's thumbnail alongside it.
func (s *ImageStore) List(ctx context.Context) ([]string, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	names, err := s.listFileNames("blobs")
	if err != nil {
		return nil, err
	}
	shas := make([]string, 0, len(names))
	for _, name := range names {
		canon, err := canonicalSHA(name)
		if err != nil {
			continue // skip stray non-blob files (e.g. leftover temp uploads)
		}
