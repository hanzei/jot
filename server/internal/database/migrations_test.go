...
// TestMigration000010LowercaseUsernames covers the upgrade path for databases
// created while usernames still allowed [a-zA-Z0-9_-]. Folding them is what
// makes the existing UNIQUE index on users.username case-insensitive in
// effect, now that validation rejects upper case on the way in.
func TestMigration000010LowercaseUsernames(t *testing.T) {
	dsntest.ForEachDriver(t, func(t *testing.T, driver string) {
		t.Run("existing mixed-case usernames are folded", func(t *testing.T) {
			db := dsntest.RawDB(t, driver)
			d := &dialect.Dialect{Driver: driver}
			ctx := t.Context()
			...
			_, err := db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000010', 'MixedCase', 'x')`)
			require.NoError(t, err)
			_, err = db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000011', 'already-lower', 'x')`)
			require.NoError(t, err)

			require.NoError(t, m.Migrate(10))

			var folded, untouched string
			require.NoError(t, db.QueryRowContext(ctx,
				d.RewritePlaceholders(`SELECT username FROM users WHERE id = ?`), "user000000000000000010",
			).Scan(&folded))
			require.NoError(t, db.QueryRowContext(ctx,
				d.RewritePlaceholders(`SELECT username FROM users WHERE id = ?`), "user000000000000000011",
			).Scan(&untouched))

			assert.Equal(t, "mixedcase", folded)
			assert.Equal(t, "already-lower", untouched, "rows already lower case are left alone")
		})

		t.Run("accounts that differ only by case abort the migration", func(t *testing.T) {
			db := dsntest.RawDB(t, driver)
			ctx := t.Context()
			...
			_, err := db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000012', 'Ben', 'x')`)
			require.NoError(t, err)
			_, err = db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000013', 'ben', 'x')`)
			require.NoError(t, err)

			// Both accounts survive: the fold cannot pick a winner, so it
			// refuses rather than merging or renaming one of them.
			require.Error(t, m.Migrate(10))

			var n int
			require.NoError(t, db.QueryRowContext(ctx, `SELECT COUNT(*) FROM users`).Scan(&n))
			assert.Equal(t, 2, n)
		})

		t.Run("checkUsernameCollisions names the colliding accounts", func(t *testing.T) {
			db := dsntest.RawDB(t, driver)
			ctx := t.Context()
			...
			require.NoError(t, checkUsernameCollisions(ctx, db), "no users yet, nothing to collide")

			_, err := db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000014', 'Dana', 'x')`)
			require.NoError(t, err)
			require.NoError(t, checkUsernameCollisions(ctx, db), "one account is not a collision")

			_, err = db.ExecContext(ctx, `INSERT INTO users (id, username, password_hash) VALUES ('user000000000000000015', 'dana', 'x')`)
			require.NoError(t, err)

			err = checkUsernameCollisions(ctx, db)
			require.Error(t, err)
			assert.Contains(t, err.Error(), `"dana" (2 accounts)`)
		})
	})
}
