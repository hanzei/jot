...
func createSharedTodoNote(t *testing.T, ts *TestServer, owner *TestUser, sharedWith *TestUser) (string, string) {
	...
	require.NoError(t, owner.Client.ShareNote(ctx, note.ID, sharedWith.User.ID))
...
func TestTaskAssignment(t *testing.T) {
	...
	t.Run("assign item to shared user on update", func(t *testing.T) {
		...
		_, err := owner.Client.UpdateNote(ctx, noteID, &jotclient.UpdateNoteRequest{
			Title: "Shared Todo",
	...
	t.Run("reject assignment on unshared note", func(t *testing.T) {
		...
		_, err = owner.Client.UpdateNote(ctx, note.ID, &jotclient.UpdateNoteRequest{
			Title: "Solo Todo",
			Items: []jotclient.UpdateNoteItem{
				{Text: "Item 1", Position: 0, IndentLevel: 0, AssignedTo: owner.User.ID},
			},
	...
	t.Run("collaborator can assign items", func(t *testing.T) {
		...
		_, err := collaborator.Client.UpdateNote(ctx, noteID, &jotclient.UpdateNoteRequest{
			Title: "Shared Todo",
			Items: []jotclient.UpdateNoteItem{
				{Text: "Item 1", Position: 0, IndentLevel: 0, AssignedTo: owner.User.ID},
				{Text: "Item 2", Position: 1, IndentLevel: 0},
...
func TestMyTodoFilter(t *testing.T) {
	t.Run("returns notes with items assigned to current user", func(t *testing.T) {
		...
		notes, err := collaborator.Client.ListNotes(ctx, &jotclient.ListNotesOptions{MyTodo: true})
		require.NoError(t, err)
	...
	t.Run("returns empty list when no assignments exist", func(t *testing.T) {
		...
		_, err := owner.Client.CreateNote(ctx, &jotclient.CreateNoteRequest{
			Title:    "Solo Todo",
			NoteType: jotclient.NoteTypeTodo,
			Items: []jotclient.CreateNoteItem{
				{Text: "Item 1", Position: 0, IndentLevel: 0},
		...
		notes, err := owner.Client.ListNotes(ctx, &jotclient.ListNotesOptions{MyTodo: true})
		require.NoError(t, err)
		assert.Empty(t, notes)
	})
	...
	t.Run("excludes trashed notes from my_todo filter", func(t *testing.T) {
		...
		require.NoError(t, owner.Client.DeleteNote(ctx, noteID))
...
func TestTaskAssignmentUnshareCleanup(t *testing.T) {
	t.Run("unshare clears unshared users assignments", func(t *testing.T) {
		...
		require.NoError(t, owner.Client.ShareNote(ctx, note.ID, collab1.User.ID))
		require.NoError(t, owner.Client.ShareNote(ctx, note.ID, collab2.User.ID))
		...
		require.NoError(t, owner.Client.UnshareNote(ctx, note.ID, collab1.User.ID))
	...
	t.Run("unshare last collaborator clears all assignments including owner", func(t *testing.T) {
		...
		require.NoError(t, owner.Client.UnshareNote(ctx, noteID, collab.User.ID))
...
func TestTaskAssignmentUserDeletion(t *testing.T) {
	t.Run("deleting a user clears their assignments across all notes", func(t *testing.T) {
		...
		require.NoError(t, admin.Client.AdminDeleteUser(ctx, collab1.User.ID))
	...
	t.Run("deleting last collaborator clears all assignments including owner self-assignment", func(t *testing.T) {
		...
		require.NoError(t, admin.Client.AdminDeleteUser(ctx, collabID))
