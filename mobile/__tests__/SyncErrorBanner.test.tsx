...
jest.mock('../src/store/OfflineContext', () => ({
  useOfflineContext: jest.fn(() => ({ isConnected: true, syncError: false, pendingNoteIds: new Set() })),
}));
...
describe('SyncErrorBanner', () => {
  beforeEach(() => {
    mockUseOfflineContext.mockReturnValue({ isConnected: true, syncError: false, pendingNoteIds: new Set() });
  });

  it('renders the sync-error message when online and sync has failed', () => {
    mockUseOfflineContext.mockReturnValue({ isConnected: true, syncError: true, pendingNoteIds: new Set() });
  ...
  it('does not render while offline (the offline banner takes over)', () => {
    mockUseOfflineContext.mockReturnValue({ isConnected: false, syncError: true, pendingNoteIds: new Set() });
    const { queryByText } = render(<SyncErrorBanner />);
    expect(queryByText(/haven't synced/i)).toBeNull();
  });
});
