...
describe('ListItem drag handle', () => {
  it('renders drag handle when showDragHandle is true and onDrag provided', async () => {
  ...
  it('calls onDrag on long press', async () => {
    const onDrag = jest.fn();
    const { getByTestId } = await render(
      <ListItem text="Task" completed={false} showDragHandle onDrag={onDrag} />,
    );

    await fireEvent(getByTestId('list-item-drag-handle'), 'onLongPress');
  ...
  it('does not render drag handle when showDragHandle is true but onDrag is not provided', async () => {
    const { queryByTestId } = await render(
