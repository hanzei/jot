...
describe('NoteImageGallery', () => {
  it('renders nothing when there are no images', async () => {
    const { toJSON } = await render(<NoteImageGallery images={[]} />);
    expect(toJSON()).toBeNull();
  });

  it('renders a full-width banner for a single image', async () => {
    const image = makeImage();
    const { getByTestId, queryByTestId } = await render(<NoteImageGallery images={[image]} />);
  ...
  it('keeps a portrait image banner full width instead of shrinking to fit its aspect ratio', async () => {
    ...
    const { getByTestId } = await render(<NoteImageGallery images={[image]} />);

    const container = getByTestId('note-image-banner-container');
    await fireEvent(container, 'layout', { nativeEvent: { layout: { width: 320, height: 0, x: 0, y: 0 } } });
  ...
  it('renders a 2-column grid for two or more images', async () => {
    const images = [makeImage({ id: 'img-1' }), makeImage({ id: 'img-2' })];
    const { getByTestId, queryByTestId } = await render(<NoteImageGallery images={images} />);
  ...
  it('folds images beyond the visible tile count into a "+N" overlay tile', async () => {
    const images = Array.from({ length: 6 }, (_, i) => makeImage({ id: `img-${i + 1}` }));
    const { getByTestId, getByText, queryByTestId } = await render(<NoteImageGallery images={images} />);
  ...
  it('opens the lightbox at the tapped image index', async () => {
    const images = [makeImage({ id: 'img-1' }), makeImage({ id: 'img-2' })];
    const { getByTestId, queryByTestId } = await render(<NoteImageGallery images={images} />);

    expect(queryByTestId('image-lightbox')).toBeNull();

    await fireEvent.press(getByTestId('note-image-grid-tile-img-2'));

    expect(getByTestId('image-lightbox')).toBeTruthy();
    expect(getByTestId('lightbox-page-img-2')).toBeTruthy();
  });

  it('closes the lightbox when its close button is pressed', async () => {
    const images = [makeImage({ id: 'img-1' })];
    const { getByTestId, queryByTestId } = await render(<NoteImageGallery images={images} />);

    await fireEvent.press(getByTestId('note-image-banner-img-1'));
    expect(getByTestId('image-lightbox')).toBeTruthy();
  ...
  describe('editable mode', () => {
    it('does not render a remove button when not editable', async () => {
      const { queryByTestId } = await render(<NoteImageGallery images={[makeImage()]} onRemove={jest.fn()} />);
      expect(queryByTestId('remove-image-img-1')).toBeNull();
    });

    it('calls onRemove with the tapped image', async () => {
      const onRemove = jest.fn();
      const image = makeImage();
      const { getByTestId } = await render(<NoteImageGallery images={[image]} editable onRemove={onRemove} />);

      await fireEvent.press(getByTestId('remove-image-img-1'));

      expect(onRemove).toHaveBeenCalledWith(image);
    });

    it('does not render a remove button on the "+N" overlay tile', async () => {
      const images = Array.from({ length: 5 }, (_, i) => makeImage({ id: `img-${i + 1}` }));
      const { queryByTestId } = await render(<NoteImageGallery images={images} editable onRemove={jest.fn()} />);
  ...
  describe('pending uploads', () => {
    ...
    it('renders an uploading tile with progress', async () => {
    ...
    it('lets an in-flight upload be cancelled from its tile (issue #695)', async () => {
      ...
      await fireEvent.press(getByTestId('dismiss-upload-upload-1'));

      expect(onDismissUpload).toHaveBeenCalledWith('upload-1');
    });

    it('does not render a cancel button on an uploading tile without an onDismissUpload handler', async () => {
    ...
    it('renders an errored upload with retry and dismiss actions', async () => {
      ...
      await fireEvent.press(getByTestId('retry-upload-upload-1'));
    ...
    it('sorts upload tiles before persisted images', async () => {
      const image = makeImage({ id: 'img-1' });
      const upload = makeUpload();
      const { toJSON } = await render(<NoteImageGallery images={[image]} editable uploads={[upload]} />);
