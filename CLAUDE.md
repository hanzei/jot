...
| `REGISTRATION_ENABLED` | `true` (unless explicitly `false`) | Whether public user registration is allowed. When `false`, only admins can create users via the admin API. The first user can always register (bootstrap). |
