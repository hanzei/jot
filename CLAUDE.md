...
**Timestamps** — every timestamp column is naive (SQLite `DATETIME`, Postgres `TIMESTAMP WITHOUT TIME ZONE`) and holds a UTC wall clock. The store layer **generates every timestamp in Go** and passes it explicitly; no store statement uses `CURRENT_TIMESTAMP`. Use `models.Now()` (never `time.Now()`) and bind `models.Timestamp(...)` of it, not the `time.Time` itself — bound directly, the SQLite driver stores Go's own `String` format, which SQLite's date functions cannot read.

Take **one `Timestamp(Now())` per logical operation** and share it across every statement in it, so all rows an operation writes carry the same value. That is what keeps the two backends in step: their own clocks differ in both precision (SQLite's `CURRENT_TIMESTAMP` is whole seconds, Postgres' is microseconds) and scope (Postgres' is transaction-constant, SQLite's is evaluated per statement). `Timestamp` formats to microseconds — Postgres' native resolution — so both backends store byte-identical text.

The `DEFAULT CURRENT_TIMESTAMP` in the schema remains only as a backstop for rows written outside the store layer, such as data migrations; it is not meant to fire for application writes. Because a forgotten column would silently fall back to it, `TestInsertsSupplyTimestampColumns` scans this package's `INSERT`s and fails when one omits a timestamp column — add new tables to `timestampColumnsByTable`. Postgres pools additionally open through a connector that runs `SET TIME ZONE 'UTC'` on every session (`internal/database`.`utcConnector`), which keeps those default-written rows UTC.
