        ...
        {note.note_type === 'text' ? (
          ...
          <div className="space-y-1">
            {(() => {
              ...
              const completedCount = (note.items?.length || 0) - uncompletedItems.length;
              const itemsToShow = uncompletedItems.slice(0, 5);
              const remainingUncompleted = uncompletedItems.length - itemsToShow.length;
                ...
                <>
                  ...
                  {remainingUncompleted > 0 && (
                    <div className="text-xs text-gray-500">
                      +{remainingUncompleted} more item{remainingUncompleted > 1 ? 's' : ''}
                    </div>
                  )}
                  {completedCount > 0 && (
                    <div className="text-xs text-gray-500">
                      +{completedCount} checked item{completedCount > 1 ? 's' : ''}
                    </div>
                  )}
                </>
              );
