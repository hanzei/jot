...
export default function CreateUserModal({ passwordMinLength, onClose, onSuccess }: CreateUserModalProps) {
  ...
  return (
    <Dialog open={true} onClose={loading ? () => {} : onClose} className="relative z-50">
      <DialogBackdrop transition aria-hidden="true" className="fixed inset-0 bg-black/25 dark:bg-black/50 duration-200 ease-out data-[closed]:opacity-0 motion-reduce:transition-none" />
      <div className="fixed inset-0 overflow-y-auto">
        <div className="flex min-h-full items-center justify-center p-4">
          <DialogPanel transition className="mx-auto w-full max-w-md rounded-lg bg-white dark:bg-slate-800 shadow-xl border border-gray-200 dark:border-slate-700 duration-200 ease-out data-[closed]:scale-95 data-[closed]:opacity-0 motion-reduce:transition-none">
